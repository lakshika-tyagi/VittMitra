    ...
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div className="badge badge-emerald">
            <CircleDot size={12} />
            <span>Step 1: Environment Initialized</span>
          </div>
        </div>
      </header>
      ...
      <section className="glass-panel" style={{ padding: '2.5rem', marginBottom: '2.5rem' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
          <span className="badge badge-blue">Development Foundation</span>
        </div>
        <h2 style={{ fontSize: '2.25rem', fontWeight: 800, lineHeight: 1.2, marginBottom: '1rem' }}>
          Intelligent Government Scheme Discovery & <span className="gradient-text">Financial Planning Engine</span>
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', maxWidth: '800px', lineHeight: 1.6, marginBottom: '2rem' }}>
          VittMitra bridges the critical gap between marginalized entrepreneurs and verified government schemes. Built on deterministic rule evaluation, PostGIS geospatial feasibility, financial structuring, and grounded Google Gemini AI explainability.
        </p>
            ...
            >
              <div
                style={{
                  ...
                  color: index === 0 ? '#34d399' : 'var(--text-muted)',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  fontSize: '0.75rem',
                  fontWeight: 700,
                  flexShrink: 0,
                  border: index === 0 ? '1px solid rgba(16, 185, 129, 0.4)' : '1px solid var(--border-subtle)',
                }}
              >
                {index + 1}
              </div>
              <span style={{ fontSize: '0.9rem', fontWeight: 500, color: index === 0 ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                {stage}
              </span>
            </div>
          ))}
